...
class JsonUserImporter(ExcelUserImporter):
    def parse_file(self, uploaded_file) -> ImportPreviewResult:
        result = ImportPreviewResult()
        ...
        try:
            ...
            result.error_rows.append(
                ImportRowResult(
                    row_number=0,
                    status="error",
                    error=f"Gagal membaca file JSON: {exc}",
                )
            )
            return result
        ...
        if not isinstance(rows, list):
            result.error_rows.append(
                ImportRowResult(
                    row_number=0,
                    status="error",
                    error="Format JSON tidak valid. Gunakan array data atau objek dengan key 'users'.",
                )
            )
            return result

        candidates = []
        for raw in rows[: self.MAX_ROWS]:
            if not isinstance(raw, dict):
                continue
            normalized = {_normalize_header(k): v for k, v in raw.items()}
            candidates.append(
                {
                    "email": _clean_value(normalized.get("email")).lower(),
                    "username": _clean_value(normalized.get("username")),
                }
            )
        existing_emails, existing_usernames = _collect_existing_accounts(candidates)
        ...
        for idx, raw in enumerate(rows, start=1):
            if not isinstance(raw, dict):
                result.error_rows.append(
                    ImportRowResult(
                        row_number=idx,
                        status="error",
                        error="Setiap item JSON harus berupa object.",
                    )
                )
                result.total_rows += 1
                continue
            ...
            normalized = {_normalize_header(k): v for k, v in raw.items()}
            clean_data = {header: normalized.get(header, "") for header in self.all_headers}
            row_result = self._validate_row_data(
                row_number=idx,
                data=clean_data,
                existing_emails=existing_emails,
                existing_usernames=existing_usernames,
